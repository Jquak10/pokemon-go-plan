...
Static assets are served from public through the ASSETS binding. Cloudflare's default asset-first behavior remains in place for ordinary JS/CSS/image requests, while `assets.run_worker_first` selectively invokes the Worker for API/calendar routes and every current HTML entry route: Landing, Data Sources, Admin, the direct Planner shell route, and private `/manage/<token>` links. This ensures response-hardening middleware actually executes for HTML without sending all static assets through Worker code.
...
The HTML CSP is route-aware but every canonical product HTML page now uses the strict external-script policy: Landing (`/`), Data Sources (`/sources`), Admin (`/admin`), the direct Planner shell (`/manage`), and private `/manage/<token>` links all use `script-src 'self'` with no `'unsafe-inline'` script allowance. Their executable application logic lives in same-origin external files, and those HTML shells must not contain executable inline `<script>` blocks or inline `on*=...` handlers. `style-src 'unsafe-inline'` remains unchanged because this hardening is intentionally limited to executable script. Selective `assets.run_worker_first` patterns cover those canonical HTML routes so Cloudflare cannot serve the pages ahead of the Worker and bypass these headers. `assets.html_handling` is explicitly `auto-trailing-slash`, so `/index.html`, `/sources.html`, `/admin.html`, and `/manage.html` remain 307 redirects to the corresponding hardened canonical routes rather than alternate HTML responses; ordinary versioned JS/CSS/image assets stay asset-first. Chromium/WebKit fixtures and deterministic Worker-boundary tests preserve the strict-script policy, while production smoke verifies both the deployed headers and canonical redirect behavior.
