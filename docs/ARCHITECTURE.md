...
Static assets are served from public through the ASSETS binding. Cloudflare's default asset-first behavior remains in place for ordinary JS/CSS/image requests and for public static HTML. Landing, Data Sources, and the direct `/manage` shell receive the security-header contract from `public/_headers`; Admin and private `/manage/<token>` routes remain selectively Worker-first, alongside API/calendar routes. This avoids routing public static HTML through Worker code solely for headers while preserving the same browser security policy.
