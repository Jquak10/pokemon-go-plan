...
Production availability is monitored by a separate `.github/workflows/production-smoke.yml` workflow so uptime checks do not increase the cadence of upstream Pokémon-data contract tests. It runs after pushes to `main`, on manual dispatch, and every three hours. The probe is read-only and uses no repository secrets: it checks hardened Landing/Data Sources/Admin and the direct Planner shell route, verifies canonical 307 redirects for direct `.html` requests, then checks a synthetic `/manage/<invalid-token>` Planner shell, every versioned same-origin script/stylesheet reference derived from the checked-out `manage.html`, and `GET /api/me` with the same fixed synthetic invalid management token. Every HTML response must expose the Worker hardening contract—strict `script-src 'self'`, no-referrer, no-sniff, frame denial, restrictive Permissions Policy, and same-origin opener/resource policies—and Admin/Planner HTML must be `no-store`. This specifically detects a regression where Cloudflare serves a matching HTML asset before the Worker because a `run_worker_first` pattern was removed. Production references must match the current repository before each asset is fetched; critical asset contracts additionally assert the theme initializer, shared semantic stylesheet, Planner-specific responsive stylesheet, and main Planner application script. The expected API `401` JSON/no-store response still proves that the deployed Worker route and D1 management-token lookup path are reachable without creating or mutating planner data or using a real capability. Short bounded retries absorb transient network failures and normal deployment overlap. The probe logic is exercised against a local deterministic HTTP fixture—including a missing-Planner-asset failure path—in the normal `npm test` suite, while the real production workflow remains intentionally outside the required PR checks.
