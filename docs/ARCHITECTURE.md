...
Theme accessibility is enforced as code rather than by screenshot comparison. `tests/theme-accessibility.test.mjs` parses the Light/Dark semantic token sets and calculates WCAG relative luminance/contrast for representative text, input, status, selected-navigation, and focus combinations. The Chromium suite independently measures rendered representative controls/status/calendar-source surfaces in both themes, then switches the theme while a modal is active to prove that focus containment, background inertness, reduced-motion handling, tab semantics, and live-region semantics survive the appearance change. Normal text pairs target at least 4.5:1 and focus indicators target at least 3:1 against representative adjacent surfaces; disabled controls remain distinguishable but are not treated as ordinary enabled-text contrast targets.
