...
The HTML CSP is route-aware but every current product HTML entry surface now uses the strict external-script policy: Landing (`/`, `/index.html`), Data Sources (`/sources`, `/sources.html`), Admin (`/admin`, `/admin.html`), direct Planner shell aliases (`/manage`, `/manage.html`), and private `/manage/<token>` links all use `script-src 'self'` with no `'unsafe-inline'` script allowance. Their executable application logic lives in same-origin external files, and those HTML shells must not contain executable inline `<script>` blocks or inline `on*=...` handlers. `style-src 'unsafe-inline'` remains unchanged because this hardening is intentionally limited to executable script. Selective `assets.run_worker_first` patterns cover the canonical extensionless HTML routes so Cloudflare cannot serve those pages ahead of the Worker and bypass these headers. `assets.html_handling` is explicitly `auto-trailing-slash`, preserving Cloudflare's 307 redirects from direct `.html` requests to the hardened canonical routes; ordinary versioned JS/CSS/image assets stay asset-first. Chromium/WebKit fixtures and deterministic Worker-boundary tests preserve the strict-script policy, while production smoke verifies the deployed headers and direct aliases.
