...
Production availability is monitored by a separate `.github/workflows/production-smoke.yml` workflow so uptime checks do not increase the cadence of upstream Pokémon-data contract tests. It runs after pushes to `main`, on manual dispatch, and every three hours. The probe is read-only and uses no repository secrets: it checks the landing HTML plus the versioned `landing-app.js` expected by the checkout, the public Data Sources page, a synthetic `/manage/<invalid-token>` Planner shell, every versioned same-origin script/stylesheet reference derived from the checked-out `manage.html`, and `GET /api/me` with the same fixed synthetic invalid management token. The Planner shell must return HTML with `no-store`; its production references must match the current repository before each asset is fetched. Critical contracts additionally assert the theme initializer, shared semantic stylesheet, Planner-specific responsive stylesheet, and main Planner application script, so a stale shell, missing asset, cache-version mismatch, or partial static deployment fails the monitor. The expected API `401` JSON/no-store response still proves that the deployed Worker route and D1 management-token lookup path are reachable without creating or mutating planner data or using a real capability. Short bounded retries absorb transient network failures and normal deployment overlap. The probe logic is exercised against a local deterministic HTTP fixture—including a missing-Planner-asset failure path—in the normal `npm test` suite, while the real production workflow remains intentionally outside the required PR checks.
