...
Static assets are served from public through the ASSETS binding. Cloudflare's default asset-first behavior remains in place for ordinary JS/CSS/image requests and for public static HTML. Landing, Data Sources, Help & Data Handling, and the direct `/manage` shell receive the security-header contract from `public/_headers`; Admin and private `/manage/<token>` routes remain selectively Worker-first, alongside API/calendar routes. This avoids routing public static HTML through Worker code solely for headers while preserving the same browser security policy.
...
The product's public data-handling boundary is intentionally explanatory rather than account-based. `/help` states what D1 planner records contain (preferences, Targets/notes, battle/resource history and credential state), what is not part of that record (email/password identity, a full Pokémon roster, browser-local appearance, or a separate server-side backup archive), and how bearer management/calendar URLs must be protected. It also states that downloaded backups can contain private notes/gameplay history, deletion cannot remove copies the user stored elsewhere, and recovery without either the management link or a previously saved backup is not available.

Planner-loss resilience uses portable backups rather than an identity/account-recovery mechanism. `GET /api/planner/backup` requires the current management capability and exports versioned JSON containing planner settings, Targets, Remote usage, current-day ceiling state if present, active/future Max tier overrides, Battle resource state/history, legacy Raid logs, and unified Battle logs. The export deliberately omits `manage_hash`, `feed_hash`, signed-calendar generation/enabled state, raw management/calendar URLs/tokens, and global event/meta data. The file can therefore reconstruct planner state but cannot authorize access to the source planner; it should still be kept private because it can contain notes and gameplay history.

`POST /api/planner/restore` also requires the current destination management capability plus exact `RESTORE` confirmation. For the current browser/header/query capability forms, the Worker validates the management capability before touching the request body; the historical body-token form remains compatible but is discovered only through the same bounded parser used for every restore. Restore request bodies are capped at 25 MB server-side: an over-limit `Content-Length` is rejected without reading the body, and streamed bodies are counted while read so a missing or deceptive length header cannot bypass the ceiling. Oversized requests return JSON 413, while malformed bounded JSON returns the stable restore-file 400. Restore is accepted only when the destination planner has no planner-owned activity rows; this prevents accidental merging/overwriting and gives the lost-link recovery path a clear flow: create a new planner, keep its newly issued capabilities, then restore the backup. Destination management/calendar credentials and signed-feed generation state are never overwritten.

Restore normalizes/validates the versioned backup, filters calendar sources through the current source catalog, keeps only a current-local-date daily Remote ceiling override and non-expired Max overrides, and remaps Target/log identifiers into a destination-planner namespace so restoring while the source planner still exists cannot collide with global primary keys. Historical orphaned Target references are namespaced too, preserving the existing intentional Undo-conflict behavior when a Target was deleted after logging.

Battle history is restored as already-applied history: restored `battle_log` rows receive non-null unique legacy markers so the `battle_log_apply` trigger does not replay target/resource/Remote effects during import. Their target/resource ledgers are restored separately, and future Undo still uses the ordinary `battle_log_undo` trigger to reverse the original deltas. Previously imported legacy-Undo relationships retain their mapped legacy ID so `unified_battle_log` preserves the source visibility semantics.

Large restores use chunked JSON bound values expanded by D1 `json_each()`. Chunks stay below the D1 bound-string/row size ceiling, and the restore refuses a plan requiring more than 45 batched statements so authentication, emptiness validation, and the transactional write remain under the Free-plan query-per-invocation ceiling. The entire destination mutation is submitted through one D1 `batch()`; a failed statement rolls back the restore. No BL-033 D1 migration, new secret, binding, route pattern, or Cloudflare Cron is required.
...
The HTML CSP is route-aware but every canonical product HTML page now uses the strict external-script policy: Landing (`/`), Data Sources (`/sources`), Help & Data Handling (`/help`), Admin (`/admin`), the direct Planner shell (`/manage`), and private `/manage/<token>` links all use `script-src 'self'` with no `'unsafe-inline'` script allowance. Their executable application logic lives in same-origin external files, and those HTML shells must not contain executable inline `<script>` blocks or inline `on*=...` handlers. `style-src 'unsafe-inline'` remains unchanged because this hardening is intentionally limited to executable script. For public/static HTML, `public/_headers` applies the strict CSP and companion response headers directly in Cloudflare Static Assets. Admin and private `/manage/<token>` responses apply the same contract through `src/http-security.js`; the direct static `/manage` shell is no-store through `public/_headers`. `assets.html_handling` is explicitly `auto-trailing-slash`, so `/index.html`, `/sources.html`, `/help.html`, `/admin.html`, and `/manage.html` remain 307 redirects to their canonical routes rather than alternate HTML responses. The Worker retains strict handling for public pages if invoked directly, but production correctness does not depend on making public static HTML Worker-first. Chromium/WebKit fixtures, deterministic static-header/Worker-boundary tests, and production smoke preserve the contract.
...
- public/help.html — public Help & Data Handling surface for planner storage boundaries, capability-link safety, backup/recovery, deletion, browser-local appearance, and safe issue reporting. It is static, credential-free, and intentionally has no support form, analytics, or identity layer.
...
Every HTML surface that uses the shared visual system—Landing, authenticated Planner, Data Sources, Help & Data Handling, and Admin—loads the same versioned `theme.js` before `styles.css`. That external same-origin script is compatible with the strict `script-src 'self'` policy and applies `data-theme` / `data-theme-preference` on the root element before CSS is parsed, avoiding a light-first flash when the resolved theme is Dark. System mode listens for `prefers-color-scheme` changes; explicit Light/Dark choices ignore later OS changes until System is selected again.

The resolved mode also sets native `color-scheme` and updates the existing `theme-color` meta tag. Major structural surfaces consume semantic tokens from `styles.css`; Pokémon/source/status identity accents keep their distinct meaning while theme-specific variants provide suitable dark backgrounds/text.

Theme accessibility is a regression invariant rather than a visual-review convention. `tests/theme-accessibility.test.mjs` parses the actual semantic Light/Dark token blocks and applies WCAG relative-luminance calculations: representative normal text, secondary text, placeholders, primary/secondary controls, and success/warning/danger/info/violet status pairs must remain at least 4.5:1; input boundaries and the keyboard focus color must remain at least 3:1. Chromium complements the token-level gate with computed-style checks on representative product, calendar/source, modal, focus, and disabled states. Appearance changes during an active dialog must preserve focus containment/restoration, tab/dialog/live-region semantics, background inertness, and the existing reduced-motion override. Whole-page screenshot diffs are intentionally not the primary theme-accessibility gate.
...
- every form control on Landing, Planner, Data Sources, Help & Data Handling, and Admin has a programmatic accessible name; placeholder text is never the only name;
...
- src/http-security.js — shared JSON/error responses, management/admin credential extraction with legacy compatibility, and static/private response hardening (route-aware CSP, frame protection, referrer policy, and no-store behavior). The helper retains an inline-compatible default for fallback compatibility, but every current product HTML entry route explicitly selects the strict external-script policy in `src/index.js`.
...
Official schedule discovery is durable across the event horizon. The newest Pokémon GO news index remains a bounded discovery input. Any explicitly pinned official page must also carry an event horizon and stops receiving pinned priority after that horizon, so old one-off event pages cannot permanently consume the bounded current-news discovery budget. Pin expiry does not delete stored evidence and does not block the separate retention path.

Still-future official supplement rows—active or previously staled—contribute their stored official `source_url` back into later syncs. This lets a row accidentally staled by the older newest-news-only behavior recover automatically when its official page is revisited, including a page whose temporary pin has already expired. Those retained URLs are normalized to approved Pokémon GO hosts, deduplicated, bounded independently from the newest-news discovery budget, and revisited until their stored event horizon ends.
...
Production monitoring consumes the same persisted source-health records through the public, credential-free, read-only `GET /api/health/data-freshness` endpoint. The endpoint evaluates every scheduled event source, the derived/current-Max evidence sources, the official schedule source, and all automatic meta dependencies. Because each source normally refreshes every six hours, production data becomes materially stale after **18 hours without a successful refresh**: one missed cycle remains `degraded` and monitor-safe while last-known-good data is fresh, while two consecutive missed cycles reach the stale threshold. Missing required health records are `unknown` and fail monitoring because there is no trustworthy freshness evidence. The public response exposes timestamps/counts/status only; raw upstream errors and source URLs remain server-side. Healthy/degraded responses return 200, while stale/unknown/unavailable responses return 503. The three-hour Production smoke calls this endpoint without credentials and fails only when the response is no longer monitor-safe.
...
A real-browser regression suite runs in Chromium for UI behavior that source inspection and VM execution cannot validate reliably. The browser fixture is self-contained: the test starts and stops its own local HTTP fixture server and uses deterministic mocked Planner/catalog responses. Regression fixtures are intentionally independent of the live event rotation: a historical species/form such as Dynamax Rhyhorn remains valid test data after it leaves live Max Battles because the contract being tested is form resolution/rendering, not current availability. Current Chromium coverage includes intermediate desktop text visibility, asynchronous Max battle-intel rendering, mobile modal containment/background locking, and horizontal page overflow.

The same required `browser-ui` job also runs a focused Playwright WebKit smoke suite against that deterministic fixture. It deliberately does not mirror the full Chromium suite: it covers the landing page and theme persistence, primary Planner shell, mobile fixed navigation and safe-area spacing, the More foreground sheet, Preferences and native backup-file controls, Calendar flow, representative phone/desktop horizontal overflow, and desktop sticky navigation/Quick Status/Calendar behavior. Keeping WebKit smoke-sized gives Safari-engine coverage for layout/input/theme risks without multiplying the entire UI regression runtime or introducing live-data flakiness.

GitHub Actions runs deterministic regression checks and the Chromium + focused WebKit browser gate automatically for every pull request and every push to `main`; no manual development server is required for the browser job. The upstream live-contract check does not gate pull requests because it depends on external availability and schema drift; it still runs on `main` pushes, schedules, and manual workflow runs. This keeps PR regression status deterministic while preserving early warning for upstream changes.

Production availability and data freshness are monitored by a separate `.github/workflows/production-smoke.yml` workflow so external production checks do not increase the cadence of upstream Pokémon-data contract tests. It runs after pushes to `main`, on manual dispatch, and every three hours. The probe is read-only and uses no repository secrets: it checks hardened Landing/Data Sources/Help/Admin and the direct Planner shell route, verifies canonical 307 redirects for direct `.html` requests, then checks a synthetic `/manage/<invalid-token>` Planner shell, every versioned same-origin script/stylesheet reference derived from the checked-out `manage.html`, the credential-free `GET /api/health/data-freshness` signal, and `GET /api/me` with the same fixed synthetic invalid management token. Every HTML response must expose the Worker hardening contract—strict `script-src 'self'`, no-referrer, no-sniff, frame denial, restrictive Permissions Policy, and same-origin opener/resource policies—and Admin/Planner HTML must be `no-store`. Production references must match the current repository before each asset is fetched; critical asset contracts additionally assert the theme initializer, shared semantic stylesheet, Planner-specific responsive stylesheet, and main Planner application script. Freshness monitoring accepts `healthy` and transient `degraded` source state while last-known-good data is under 18 hours old, but fails on `stale`, `unknown`, or unavailable source health. The expected API `401` JSON/no-store response still proves that the deployed Worker route and D1 management-token lookup path are reachable without creating or mutating planner data or using a real capability. Short bounded retries absorb transient network failures and normal deployment overlap. Deterministic tests cover healthy, degraded-but-fresh, stale, missing-health, missing-asset, and read-only behavior, while the real production workflow remains intentionally outside the required PR checks.
