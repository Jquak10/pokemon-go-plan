...
BL-031 — Production smoke monitoring — is implemented by the current change and removed from Active in the same change, consistent with the backlog rules. A dedicated read-only GitHub Actions workflow checks the live landing page, its versioned landing asset, Data Sources, and a synthetic-invalid authenticated API lookup after `main` pushes, on manual dispatch, and every three hours.
...
A fresh public-readiness audit on 23 September 2026 identified whole-planner self-service deletion, fixed Singapore timezone onboarding, and the absence of repository-owned production smoke monitoring as concrete public-launch gaps. The user explicitly selected BL-029 on 24 September 2026; it shipped in PR #77 with management-authenticated permanent deletion backed by the existing `ON DELETE CASCADE` schema. The user then selected BL-030, which shipped in PR #78 with validated browser timezone detection while preserving manual control and refusing to silently assume another region when detection is unavailable. The user explicitly selected BL-031 on 24 September 2026; the current change adds scheduled, post-`main`, and manually runnable read-only production smoke checks with deterministic local fixture coverage. Other launch-hardening ideas from that audit remain suggestions only and are not promoted into the durable Active backlog unless the user explicitly selects them.
