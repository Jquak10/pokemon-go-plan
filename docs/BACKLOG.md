...
### BL-019 — Correct half-level Hundo CP multipliers

**Priority:** High · correctness

The Hundo calculator accepts custom levels in 0.5-level steps, but `planner-hundo-logic.js` currently derives half-level CP multipliers with a simple arithmetic average of the adjacent whole-level multipliers. Pokémon GO half-level CPM values do not use that interpolation, so some custom half-level Hundo CP results are wrong while the standard integer benchmarks remain correct.

Done when:

- half-level CPM/CP calculations follow the Pokémon GO/Game Master formula or canonical values;
- existing whole-level results remain unchanged;
- deterministic regressions cover representative half levels, including a low level where the current error is easy to detect.
