...
BL-032 — Public planner storage growth bounds — is implemented by the current change and removed from Active in the same change, consistent with the backlog rules. The Worker bounds per-planner Target, battle-log, note, and Max override growth; restricts the one-day Remote ceiling override to the current planner-local date; and prunes obsolete override rows without requiring a schema migration.
...
A fresh public-readiness audit on 23 September 2026 identified whole-planner self-service deletion, fixed Singapore timezone onboarding, repository-owned production smoke monitoring, and unbounded planner-owned storage growth as concrete public-launch gaps. The user explicitly selected BL-029 on 24 September 2026; it shipped in PR #77 with management-authenticated permanent deletion backed by the existing `ON DELETE CASCADE` schema. BL-030 shipped in PR #78 with validated browser timezone detection. BL-031 shipped in PR #79 with scheduled/post-`main` read-only production smoke monitoring. The user then explicitly selected BL-032 on 24 September 2026; the current change bounds persistent Target, battle-log, target-note, Max override, and dated Remote-ceiling override growth without adding a migration. Other launch-hardening ideas from that audit remain suggestions only and are not promoted into the durable Active backlog unless the user explicitly selects them.
