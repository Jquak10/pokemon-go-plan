...
Progress: **BL-011A** extracted the Planner capability/API/auth layer plus shared escaping/number-format utilities into `public/planner-client.js`. **BL-011B** extracted pure Target progress, availability, filtering, sorting, status counts, and grouping/view-model logic into `public/planner-target-logic.js`. **BL-011C** extracted pure Calendar UTC date/month helpers, event-range matching, source-class normalization, and month-grid projection into `public/planner-calendar-logic.js`. **BL-011D** extracted Hundo CP/formula, benchmark generation, and search ranking into `public/planner-hundo-logic.js`. **BL-011E** extracts Battle Plan/resource recommendation filtering, allocation lookup, zero-allocation compatibility/reasons, and MP/tier display metadata into `public/planner-battle-plan-logic.js`. Domain DOM rendering/state remains in `manage.html`. BL-011 remains active for Battle intel, CSS organization, and Worker orchestration slices.
