...
A fresh post-PR-#91 product audit identified BL-041 as a verified production-routing security mismatch: the Worker defined hardened public HTML responses, but Cloudflare static assets are asset-first unless the path is covered by `assets.run_worker_first`. BL-041 is implemented by the current change with selective Worker-first coverage for every canonical HTML entry path, explicit canonical `.html` redirect handling, deterministic configuration/runtime assertions, and production smoke verification of both the deployed security headers and redirect behavior. No D1 migration or all-assets Worker-first routing is required.
