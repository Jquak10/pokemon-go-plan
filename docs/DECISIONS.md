...
| #72 | BL-025 single-target deletion failure UX | Handles single-target delete failures locally in Planner Targets, reports progress/failure in an `aria-live` status region, preserves the target/search/tab context on failure, retains reload-on-success behavior, and covers both outcomes in Chromium. |
