...
| #103 | BL-050 mobile touch-target contract | Enforces 44×44 mobile hitboxes in the Planner-only responsive layer for compact buttons, form controls, collapsible summaries, range inputs, Battle Plan controls, and navigation while preserving desktop density; adds computed-size Chromium regressions across representative mobile surfaces and bumps planner.css to v6 without changing shared styles.css. |
