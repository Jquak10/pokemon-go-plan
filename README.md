...
Appearance is device/browser-local rather than planner data. The **Appearance** control is available on the Landing, Planner, Data Sources, Help & Data Handling, and Admin surfaces with **System**, **Light**, and **Dark** choices. System is the default and follows the browser/OS color preference; choosing Light or Dark is remembered in that browser. Appearance is not stored in D1, included in Planner Backup, or synchronized through the private management link.

The public **Help & Data Handling** page at `/help` explains what planner-owned data is stored, what remains browser-local or is not part of the planner record, how management/calendar bearer URLs work, backup/recovery and permanent deletion, and how to report incorrect data or application defects safely. Support uses the repository's public GitHub issue tracker; users are explicitly told never to include private management/calendar URLs, Admin keys, Planner Backup files, or screenshots that reveal them.
...
The management dashboard is accessed through each planner's private capability link. It controls that planner's Battle Plan, targets, logs, preferences, and calendar; it is not a public account profile. Keep the management URL private. The current UI sends its capability to management APIs in an authorization header rather than repeating it in request URLs or JSON bodies; the Worker still accepts the older token forms so existing integrations are not broken. Landing, Data Sources, and Help & Data Handling remain Cloudflare Static Assets for low-latency delivery, with the same CSP/frame/referrer/content/permissions/opener/resource hardening applied by `public/_headers`. Admin and private `/manage/<token>` routes remain Worker-first and receive the same policy from `src/http-security.js`; the direct `/manage` static shell is also protected by `public/_headers` and remains no-store. Cloudflare's explicit `auto-trailing-slash` HTML handling keeps direct `.html` requests as 307 redirects to the hardened extensionless routes.
...
The public **Data Sources & Precedence** page explains why explicit official schedules take priority over suppression/replacement notices and general GO Calendar data, and identifies the analytical inputs and versioned computation used for raid value and attacker rankings. The adjacent **Help & Data Handling** page documents planner storage, bearer-link safety, recovery/deletion, browser-local appearance, and the safe public issue-reporting path.
...
- The public landing page, Data Sources page, Help & Data Handling page, and Battle Planner Admin page collapse their multi-column layouts for narrow browsers, keep long URLs/text wrapped, and use touch-friendly controls without reserving space for the Planner-only bottom navigation.
...
- Public Static Asset HTML (Landing, Data Sources, Help & Data Handling, and the direct Planner shell) receives the same strict security-header contract through `public/_headers`, while Admin and private Planner links remain Worker-hardened. All current HTML executes application JavaScript only from same-origin external files under `script-src 'self'`; direct `.html` requests keep Cloudflare's canonical 307 redirects, and ordinary JS/CSS/image assets remain asset-first for performance. Inline executable scripts are regression-tested against reintroduction.
...
- The required `browser-ui` gate runs the full deterministic-fixture Chromium regression suite plus a much smaller Playwright WebKit smoke suite covering the landing page, Planner shell, mobile navigation/More sheet, Preferences and backup-file controls, Calendar, theme persistence, horizontal overflow, mobile fixed/safe-area spacing, and desktop sticky behavior. The WebKit layer is intentionally smoke-sized rather than a duplicate of Chromium.
...
Every pull request and every push to `main` runs the deterministic Planner regression workflow. Pull requests run the Node regression suite and real Chromium browser suite; the upstream live-contract job is intentionally excluded from PR gating so an external outage or upstream drift cannot make an otherwise deterministic UI/code PR flaky. The Node suite includes an explicit Light/Dark theme-accessibility gate: representative semantic text/status/button/input pairs must meet WCAG AA normal-text contrast, input/focus boundaries must meet the 3:1 non-text threshold, and the browser suite exercises those tokens on real Planner components plus calendar/source indicators. Chromium also verifies that changing appearance does not steal dialog focus, weaken tab/dialog/live-region semantics, or bypass the reduced-motion override. Pushes to `main`, scheduled runs, and manual workflow runs still exercise the live contract. The browser job installs Chromium in CI and the test itself starts and stops its own local fixture server, so **you do not need to start Wrangler or any other server for browser tests**.

Production availability and data freshness are monitored separately by the **Production smoke** GitHub Actions workflow. It runs after pushes to `main`, can be started manually, and is scheduled every three hours. The smoke probe is intentionally read-only: it verifies the landing page and versioned landing JavaScript, Data Sources, Help & Data Handling, Admin, canonical redirects for direct `.html` requests, a synthetic private Planner shell at `/manage/<invalid-token>`, **every versioned static asset referenced by the current repository's Planner shell**, the public credential-free `GET /api/health/data-freshness` signal, and an invalid synthetic management lookup that must return the normal `401` JSON response. Every HTML entry probe asserts the deployed Worker hardening contract—strict same-origin script CSP, no-referrer, no-sniff, frame denial, restrictive Permissions Policy, and same-origin opener/resource policies—while Admin/Planner shells must additionally remain `no-store`. Critical asset checks explicitly cover the shared theme initializer, shared CSS, Planner-specific CSS, and main Planner JavaScript, while automatically following the rest of the versioned Planner asset references. Freshness health is derived from the existing D1 `sync_source_health` records across scheduled event feeds, official schedules, and automatic meta inputs. Because those syncs run every six hours, one failed cycle is allowed to remain degraded while last-known-good data is still fresh; **18 hours without a successful refresh** is treated as materially stale and fails the smoke. Missing/unavailable required health records also fail because freshness cannot be proven. The public health response contains status/timestamps/counts only—no management/calendar credentials, raw upstream errors, or private planner data. Short retries absorb transient network failures and normal deployment overlap; a failed workflow run is the operational signal and notification delivery follows the repository owner's GitHub Actions notification settings. This production check is not a pull-request gate; healthy, degraded, stale, missing-health, and missing-asset behavior is covered locally by deterministic tests instead.
