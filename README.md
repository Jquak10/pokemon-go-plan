...
Production availability is monitored separately by the **Production smoke** GitHub Actions workflow. It runs after pushes to `main`, can be started manually, and is scheduled every three hours. The smoke probe is intentionally read-only: it verifies the landing page and versioned landing JavaScript, Data Sources, Admin, canonical redirects for direct `.html` requests, a synthetic private Planner shell at `/manage/<invalid-token>`, **every versioned static asset referenced by the current repository's Planner shell**, and an invalid synthetic management lookup that must return the normal `401` JSON response. Every HTML entry probe asserts the deployed Worker hardening contract—strict same-origin script CSP, no-referrer, no-sniff, frame denial, restrictive Permissions Policy, and same-origin opener/resource policies—while Admin/Planner shells must additionally remain `no-store`. Critical asset checks explicitly cover the shared theme initializer, shared CSS, Planner-specific CSS, and main Planner JavaScript, while automatically following the rest of the versioned Planner asset references. The API check exercises the deployed Worker and D1 lookup path without creating a planner or using a real management/calendar credential. Short retries absorb transient network or deployment overlap; a failed workflow run is the operational signal and notification delivery follows the repository owner's GitHub Actions notification settings. This production check is not a pull-request gate; its behavior—including a missing-Planner-asset failure case—is covered locally by deterministic fixture tests instead.
