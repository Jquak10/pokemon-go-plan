...
- Landing, Data Sources, Admin, and private Planner HTML surfaces are selectively routed through the Worker before static assets, including direct `.html` aliases, and execute application JavaScript only from same-origin external files under `script-src 'self'`; static JS/CSS/image assets remain asset-first for performance. Inline executable scripts are regression-tested against reintroduction.
