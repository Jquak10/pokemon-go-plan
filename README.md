...
The management dashboard is accessed through each planner's private capability link. It controls that planner's Battle Plan, targets, logs, preferences, and calendar; it is not a public account profile. Keep the management URL private. The current UI sends its capability to management APIs in an authorization header rather than repeating it in request URLs or JSON bodies; the Worker still accepts the older token forms so existing integrations are not broken. Landing, Data Sources, Admin, and Planner canonical HTML entry routes are selectively Worker-first so response hardening cannot be bypassed by Cloudflare's normal static-asset-first routing. Cloudflare's explicit `auto-trailing-slash` HTML handling keeps direct `.html` requests as 307 redirects to those hardened extensionless routes. Planner/Admin HTML is additionally served no-store.
...
- Landing, Data Sources, Admin, and private Planner canonical HTML routes are selectively routed through the Worker before static assets and execute application JavaScript only from same-origin external files under `script-src 'self'`; direct `.html` requests keep Cloudflare's canonical 307 redirects, while static JS/CSS/image assets remain asset-first for performance. Inline executable scripts are regression-tested against reintroduction.
